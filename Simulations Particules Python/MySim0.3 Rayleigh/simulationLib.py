...
def generateBallsSquareInGrid(grid,dimensions,numbOfBalls,SquareA,SquareB,mass,radius, velocity):
    ...
    if(abs(round(coeff*SquareDimensions.x)-coeff*SquareDimensions.x)>0.01):
        print("NUMB OF BALLS ROUNDED!!")
...
def addGravitySpeedToBalls(balls, gravity, deltaTime):
    for aBall in balls:
        aBall.velocity.y-=gravity*deltaTime
...
def HeatandCoolGrid(grid,thickness,heatcoeff,coolcoeff):
    """
    grid (list) the grid in which the ball will be placed\n
    thickness (float) 0-1 part of the fluid from top an bottom which will be cooled/ heated
    """
    gridDimensions=getGridDimensions(grid)
    for y in range(gridDimensions.y):
        if(y<gridDimensions.y*thickness):
            for x in range(gridDimensions.x):
                print("heat")
                HeatBalls(grid[x][y],heatcoeff)
        if(y>gridDimensions.y*(1-thickness)):
            for x in range(gridDimensions.x):
                print("cool")
                CoolBalls(grid[x][y],coolcoeff)
                
def HeatBalls(balls,heatcoeff):
    for aBall in balls:
        imaginaryBall=Ball(aBall.position+Vector2((random.random()*2-1)*aBall.radius/2,(random.random()*2-1)*aBall.radius/2),aBall.mass,aBall.radius,Vector2((random.random()*2-1)*heatcoeff,(random.random()*2-1)*heatcoeff))
        collideBalls(imaginaryBall,aBall,None,Vector2(False,False))
def CoolBalls(balls,coolcoeff):
    for aBall in balls:
        imaginaryBall=Ball(aBall.position+Vector2((random.random()*2-1)*aBall.radius/2,(random.random()*2-1)*aBall.radius/2),aBall.mass,aBall.radius,Vector2((random.random()*2-1)*coolcoeff,(random.random()*2-1)*coolcoeff))
        collideBalls(imaginaryBall,aBall,None,Vector2(False,False))
...
def appendBallsDataOnFile(_balls, _file,dimensions):
