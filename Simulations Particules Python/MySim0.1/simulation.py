...
xLimit=500
yLimit=500
numbOfBallsToGen=2500
ballsRadius=3
ballsMass=1
fps=60
fixedDelay=0.02
issquare=False
dataStorageFilePath="C:/Users/lcach/Documents/4-Ecole/Physique/TM/Simulations Particules Python/MySim0.1/Data/simulationdata.txt"
...
thetime=time.time()
storageData=open(dataStorageFilePath,"w")
storageData.write("")
storageData.close()
storageData=open(dataStorageFilePath,"a")

numbOfFrames=2000
FrameIndex=0
while FrameIndex<numbOfFrames:
    FrameIndex+=1
    print(FrameIndex)
    start=time.time()
    if(FrameIndex%1==0):
        print(FrameIndex/ numbOfFrames)
    
    #print(str(timedelay)+" : "+str(actualTime)+" : "+str(thetime))
    checkBallCollisions()
    UpdateBallPositions(fixedDelay)
    setBallsSpeed()
    storageData.write("!")
    for ball in Balls:
        storageData.write("#p"+str(ball.Position.x)+"x"+str(ball.Position.y))
        storageData.write("v"+str(ball.Velocity.x)+"x"+str(ball.Velocity.y))
        storageData.write("m"+str(ball.mass))
